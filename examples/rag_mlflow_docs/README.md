...
omnigent run examples/rag_mlflow_docs/

# Override model and harness at the command line
omnigent run examples/rag_mlflow_docs/ --model gpt-4o --harness openai-agents

# Anthropic Claude (stricter RAG behavior -- see Known Limitation below)
export $(grep ANTHROPIC_API_KEY .env | tr -d '"')
omnigent run examples/rag_mlflow_docs/ --model claude-sonnet-4-6 --harness claude-sdk

# Fresh session (no persistence)
omnigent run examples/rag_mlflow_docs/ --no-session
```
